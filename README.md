...
#### ✨ Added
- Cross-platform support for macOS with custom codesigning and keychain management
- Platform detection using `platform.system()` with specialized execution paths
- MacOS-specific setup function to handle entitlements and codesigning requirements
- `sudo_run()` function to run elevated commands on macOS via AppleScript
- Improved file structure with "scripts" directory instead of "inject" for better organization
- Plist handling for macOS app entitlement modification

#### 🛠️ Changed
- Refactored platform-specific code with dynamic constants based on OS detection
- Updated RidiProcess class to handle launch differences between Windows and macOS
- Enhanced process detection to work reliably across platforms
- Modified path handling to accommodate OS-specific locations and conventions
- Improved error messages to be platform-aware
- Restructured execution flow with platform-specific checks and initializations

#### 🧹 Removed
- Windows-only assumptions throughout the codebase
- Hard-coded Windows-specific process identification
- Direct Windows registry dependencies when running on macOS
...
#### ✨ Added
- Integrated Frida for runtime JavaScript injection, eliminating the need for application patching
- Added `remote-debugging.js` for enabling Electron DevTools directly during application startup
- Implemented psutil library for more reliable process management and lifecycle control
- Added structured logging system with configurable levels and file output
- Added more robust error handling for network requests and file operations

#### 🛠️ Changed
- Completely rewrote process handling to use psutil instead of subprocess and taskkill commands
- Refactored JavaScript injection to use preloaded script files instead of embedding directly
- Optimized novel download logic to no longer depend on iframe content traversal
- Improved debugger connection management with better retry and recovery mechanisms
- Enhanced console output collection with separate tracking for errors, warnings, and logs

#### 🧹 Removed
- Deprecated and removed patch.py as Frida injection now handles DevTools enablement without patching
- Removed dependency on iframe document inspection for extracting EPUB download paths
- Removed unreliable iframe-based navigation, which could fail in certain edge cases
