...
async function init() {
    try {
                  ...
                  try {
                      ...
                      for (let i = 0; i < iframes.length; i++) {
                          try {
                              ...
                              if (
                                  iframeDoc.head.querySelector("base") !== null
                              ) {
                                  return iframes[i]; // Return the first iframe with a base tag
                              }
                          } catch (e) {
                              // Skip iframes that can't be accessed due to same-origin policy
                              continue;
                          }
                      }

                      // If no matching iframe is found, show alert and return null
                      showPopup(
                          "Error: No iframe with <base> tag in head found",
                          "error",
                          5000
                      );
                      return -1;
        ...
        btn.addEventListener("click", () => {
            if (isNovel) {
                const baseUrl = doc.querySelector("head > base")?.href; /// bug here. see 0
                if (baseUrl)
                    getEpub(
                        progress,
                        baseUrl,
        ...
        if (isNovel && iframe) {
            iframe.contentDocument?.readyState === "complete"
                ? addButton()
                : iframe.addEventListener("load", addButton);
        } else {
