...
JSZIP_JS = read_file("inject/jszip.js")
INJECT_JS = read_file("inject/inject.js")
REMOTE_DEBUGGING_JS = read_file("inject/remote-debugging.js")
...
class RidiProcess:
    ...
    async def __aenter__(self) -> subprocess.Popen:
        ...
        logger.debug(
            "[RidiBooks] Injecting into RidiBooks process pid: %s", self.process.pid
        )
        try:
            frida.attach(self.process.pid).create_script(REMOTE_DEBUGGING_JS).load()
        ...
        logger.debug(
            "[RidiBooks] Injected into RidiBooks process pid: %s", self.process.pid
        )
