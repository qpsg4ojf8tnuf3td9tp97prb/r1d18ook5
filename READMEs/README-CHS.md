...
#### ✨ 新增
- 集成 Frida 实现运行时 JavaScript 注入，无需再对应用程序进行补丁处理
- 新增 `remote-debugging.js`，可在应用启动时直接启用 Electron 开发工具
- 引入 psutil 库，提供更可靠的进程管理和生命周期控制
- 添加结构化日志系统，支持日志级别配置和文件输出
- 增强网络请求与文件操作的错误处理能力

#### 🛠️ 变更
- 完全重构进程处理逻辑，改用 psutil 替代 subprocess 与 taskkill 命令
- 重构 JavaScript 注入逻辑，改用预加载脚本文件，避免直接嵌入代码
- 优化小说下载逻辑，不再依赖 iframe 内容遍历
- 改进调试器连接管理，增加重试与恢复机制
- 增强控制台输出收集，错误、警告与普通日志分别追踪记录

#### 🧹 移除
- 废弃并删除 patch.py 文件，现已通过 Frida 注入方式启用 DevTools，无需打补丁
- 移除对 iframe 文档检查的依赖，不再通过 iframe 获取 EPUB 下载路径
- 删除不稳定的 iframe 导航方式，避免部分特殊情况导致失败
