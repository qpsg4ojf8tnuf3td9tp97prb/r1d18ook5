...
0. **Ridibooks ビューアを公式サイトからダウンロード・インストール**:  
   https://getapp.ridibooks.com/windows

1. **`inject.exe` をダウンロード**:
...
#### ✨ 追加
- Fridaを統合し、実行時のJavaScriptインジェクションを可能にしました（アプリケーションのパッチ適用不要）
- Electron開発ツールを起動時に直接有効化する`remote-debugging.js`を追加
- より信頼性の高いプロセス管理とライフサイクル制御のため、psutilライブラリを導入
- ログレベルの設定やファイル出力が可能な構造化ロギングシステムを追加
- ネットワークリクエストとファイル操作のエラーハンドリングを強化

#### 🛠️ 変更
- プロセス管理の仕組みを全面的に書き換え、subprocessやtaskkillの代わりにpsutilを使用
- JavaScriptインジェクションを、直接埋め込みからプリロードスクリプトファイルの方式に再構築
- 小説のダウンロード処理を最適化し、iframeのコンテンツ走査への依存を撤廃
- デバッガ接続管理を改善し、再試行および復旧メカニズムを強化
- コンソール出力収集を改善し、エラー・警告・ログを個別に追跡可能に

#### 🧹 削除
- FridaのインジェクションによってDevToolsを有効化するため、patch.pyを廃止・削除（パッチ処理不要化）
- EPUBダウンロードパス取得のためのiframe文書解析への依存を撤廃
- 特定状況下で失敗を引き起こす可能性がある、不安定なiframeベースのナビゲーションを削除
...
#### ✨ 新機能
- プロセスの起動フローを`RidiProcess`非同期コンテキストマネージャで完全に書き直し、ライフサイクル管理を改善
- 非同期ログタスク（`_log_process_output`）を使ったサブプロセス出力ログの改善
- `execute_js()` とデバッガーポーリングに対するWebSocketエラー処理を強化
- 終了時に残った全てのRidibooksプロセスを自動でクリーンアップ（`terminate_process()`）
- コンソールコレクタが警告を別途収集し、`__enableConsoleOutput`でコンソール出力の切替えが可能に

#### 🛠️ 変更点
- より迅速なデバッガ検出のため、`DebuggerMonitor.polling_interval`を1.0秒から0.1秒に変更
- `setup_logger()`のフォーマットを読みやすい`[LEVEL] メッセージ`形式に改善
- `main()`を再設計し、Ridibooksのクリーン起動とエラー（EXE欠落、タイムアウトなど）を安全に処理
- 注入したJSがビューアのiframe完全読み込みを保証するために60フレーム待機
- より適切なタイミングと安定性を得るために、`inject_to_viewer()`と関連する挿入フローを再構築

#### 🧹 削除
- より簡潔なログ表示のため、冗長なタイムスタンプ付きログ形式を廃止
- コンソール出力が常に有効であるという固定仕様を削除（切替可能に変更）
