...
#### ✨ 新增
- 整合 Frida 實現運行時 JavaScript 注入，無需再對應用程式進行補丁處理
- 新增 `remote-debugging.js`，可於應用程式啟動時直接啟用 Electron 開發工具
- 引入 psutil 函式庫，提供更可靠的程序管理與生命週期控制
- 新增結構化日誌系統，支援日誌級別設定與檔案輸出
- 增強網路請求與檔案操作的錯誤處理能力

#### 🛠️ 變更
- 完全重構程序處理邏輯，改用 psutil 取代 subprocess 與 taskkill 指令
- 重構 JavaScript 注入邏輯，改用預載腳本檔案，避免直接嵌入程式碼
- 優化小說下載邏輯，不再依賴 iframe 內容遍歷
- 改進除錯器連線管理，增加重試與恢復機制
- 增強主控台輸出收集，錯誤、警告與一般日誌分別追蹤記錄

#### 🧹 移除
- 廢棄並移除 patch.py 檔案，現已透過 Frida 注入方式啟用 DevTools，無需打補丁
- 移除對 iframe 文件檢查的依賴，不再透過 iframe 取得 EPUB 下載路徑
- 移除不穩定的 iframe 導航方式，避免部分特殊情況導致失敗
